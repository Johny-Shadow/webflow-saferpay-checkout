export async function sendOrderMail(order) {
  try {
    ...
    const productList = (order.items || [])
      .map(i => `• ${i.name} × ${i.quantity} – CHF ${i.price}`)
      .join('<br>');
    ...
    // ------------------------
    // 3) Mail-Template (Kunden-Vorlage)
    // ------------------------
    const mailHtml = `
      ...
      Gesamtbetrag: CHF ${order.amount}<br>
      Zahlungsmethode: Saferpay</p>
      ...
      <p>
        Liebe Grüsse<br>
        <strong>Dein Payyap-Team</strong>
      </p>
    `;
